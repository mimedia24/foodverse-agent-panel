...
import { Plus, Trash2, CreditCard, MapPin, Phone, Mail, ChevronLeft, ChevronRight, User } from "lucide-react";
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border uppercase tracking-wider ${configs[status] || configs.offline}`}>
      {status}
    </span>
  );
};
    ...
    <Layout>
      <div className="p-4 md:p-8 bg-[#F9FAFB] min-h-screen">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-black text-gray-900 tracking-tight">Rider Fleet</h1>
            <p className="text-sm text-gray-500 font-medium">
              Zone <span className="text-indigo-600">#{user?.zoneId}</span> • {apiResponse?.totalRiders || 0} Total Riders
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Link to={'/rider-register'}>
              <Button type="primary" size="large" icon={<Plus size={18} />} className="rounded-xl font-bold h-11 shadow-indigo-100 shadow-lg">
                Register New Rider
              </Button>
            </Link>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Pending Payout', value: '$1,200', color: 'text-orange-600' },
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {riders.map((rider) => (
                <div key={rider._id} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md transition-all flex flex-col overflow-hidden group">
                  ...
                  <div className="p-6 flex items-start gap-5">
                    <div className="relative shrink-0">
                      ...
                      />
                      <div className={`absolute -bottom-1 -right-1 w-5 h-5 border-4 border-white rounded-full ${rider.riderStatus === "active" ? "bg-green-500" : "bg-red-500"}`} />
                    </div>

                    <div className="flex-grow min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-bold text-lg text-gray-900 truncate capitalize">{rider.name}</h3>
                        <StatusBadge status={rider.session} />
                      </div>
                      <p className="text-xs font-mono text-gray-400 mb-3 uppercase tracking-tighter">ID: {rider._id}</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 rounded-lg text-gray-500 text-xs font-medium">
                          <Tag color="blue" className="m-0 border-none bg-transparent p-0 lowercase">{rider.riderType}</Tag>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="px-6 grid grid-cols-2 gap-4">
                    <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-blue-50/50">
                      <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">Total Earnings</p>
                      <p className="text-xl font-black text-blue-700">৳ {rider.earning}</p>
                    </div>
                    <div className="bg-[#FDF2F8] p-4 rounded-2xl border border-pink-50/50">
                      <p className="text-[10px] font-bold text-pink-400 uppercase tracking-widest mb-1">Cash Collection</p>
                      <p className="text-xl font-black text-pink-700">৳ {rider.cashCollection}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                      <div className="flex gap-2">
                        ...
                        >
                          <Button danger type="text" icon={<Trash2 size={18} />} className="rounded-xl" />
                        </Popconfirm>
                      </div>
            ...
            <div className="mt-12 flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
              <p className="text-sm text-gray-500 font-medium ml-2">
                Showing <span className="text-gray-900 font-bold">{(page-1)*limit + 1} - {Math.min(page*limit, apiResponse?.totalRiders || 0)}</span> of {apiResponse?.totalRiders || 0}
              </p>
