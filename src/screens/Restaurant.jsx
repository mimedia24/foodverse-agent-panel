    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6 md:p-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-2 font-medium">
            Zone <span className="text-indigo-600 font-semibold">{user?.zoneId}</span> • {data?.total || 0} active partners
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-14">
        {[
          ...
          >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center bg-${item.color}-50 text-${item.color}-600 mb-4`}>
              {item.icon}
            </div>
